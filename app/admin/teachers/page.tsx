...
export default async function TeachersPage() {
  const [teachers, allCourses, allSubjects, assignments, settings] = await Promise.all([
    db.select().from(users).where(and(eq(users.role, "teacher"), eq(users.isActive, true))).orderBy(asc(users.name)),
  ...
  return (
    <div className="p-8">
      <h1 className="text-xl font-semibold">Profesores y asignaciones</h1>
      ...
      <form action={createTeacherAssignment} className="mt-6 flex max-w-3xl flex-wrap items-end gap-2 text-sm">
      ...
      {teachers.length === 0 && (
        <p className="mt-4 text-sm text-zinc-500">
          Aún no hay profesores registrados: deben iniciar sesión con Google
          al menos una vez para aparecer aquí.
        </p>
      )}
    </div>
  );
