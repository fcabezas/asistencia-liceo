...
import { db } from "@/db";
import { scheduleBlocks, subjects, users, attendanceRecords } from "@/db/schema";
...
export default async function InspectorDashboard() {
  ...
  const courseName = (id: number) => scopedCourses.find((c) => c.id === id)?.name ?? id;
  ...
  return (
    <div className="p-8">
      <h1 className="text-xl font-semibold">Asistencia de bloque 1 no tomada</h1>
      ...
      {scopedCourses.length === 0 ? (
        ...
        <table className="mt-6 w-full max-w-2xl text-left text-sm">
          ...
          <tbody>
            {pending.map((b) => (
              <tr key={b.id} className="border-b bg-red-50 dark:bg-red-950/40">
